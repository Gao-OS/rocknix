...
# Copyright (C) 2026 GaoOS contributors
# Generate GaoOS release body with dynamic device tables from config.xml
...
get_image_filename() {
  ...
  echo "GaoOS-${device}.aarch64-${DATE}${suffix}.img.gz"
}

get_update_filename() {
  local device="$1"
  echo "GaoOS-${device}.aarch64-${DATE}.tar"
}
...
#
GaoOS is a community-developed Linux distribution for handheld gaming devices, forked from [ROCKNIX](https://github.com/ROCKNIX/distribution). GaoOS adds an **A/B dual-slot boot system** for atomic, rollback-safe OTA updates.
...
## Licenses
GaoOS is a Linux distribution that is made up of many open-source components. Components are provided under their respective licenses. This distribution includes components licensed for non-commercial use only.

### GaoOS Branding
GaoOS branding and images are licensed under a [Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License](https://creativecommons.org/licenses/by-nc-sa/4.0/).
...
echo "Copyright (C) $(date +%Y) GaoOS contributors"
...
## Installation
* Download the latest version of GaoOS.
...
for device in ${devices}; do
  ...
  for variant in ${variants}; do
    ...
    echo "| **${description}** | [${img_file}](https://github.com/Gao-OS/rocknix/releases/download/${DATE}/${img_file}) | ${doc_link} |"
  done
done
...
* If you are unable to update online
  * Download the latest version of GaoOS from GitHub
...
for device in ${devices}; do
  ...
  echo "| **${all_descriptions}** | [${update_file}](https://github.com/Gao-OS/rocknix/releases/download/${DATE}/${update_file}) |"
done
