...
RUN if [ "$(uname -m)" = "aarch64" ]; then \
  apt-get install -y libc6-amd64-cross libc6-dev-amd64-cross qemu-user-binfmt --no-install-recommends; \
 fi
RUN if [ ! -d /lib64 ]; then ln -sf /usr/x86_64-gaoos-linux-gnu/lib64 /lib64; fi
RUN if [ ! -d /lib/x86_64-gaoos-linux-gnu ]; then ln -sf /usr/x86_64-gaoos-linux-gnu/lib /lib/x86_64-gaoos-linux-gnu; fi
